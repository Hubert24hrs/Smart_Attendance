...
def get_my_notifications(
    ...
    return {
        ...
        "notifications": sorted(notifications, key=lambda x: x["sent_at"], reverse=True)[:50]
    }
    ...
    for student in students:
        ...
        if rate < threshold and total >= 3:  # At least 3 sessions
            ...
            notification_store[current_user.id].append({
                ...
                "message": f"Student {student.full_name} has {rate:.1f}% attendance ({present}/{total} sessions)",
                "sent_at": datetime.utcnow().isoformat(),
                "read": False
            })
            alerts_sent += 1
