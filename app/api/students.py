...
async def enroll_student(
    ...
    for file in files:
        ...
        if (
            len(encodings) == 1
        ):  # Strict: Only assume correct face if exactly 1 face found
            # Store embedding
            ...
            print(
                f"Warning: Multiple faces in enrollment image for {full_name}. "
                "Skipping."
            )
        else:
            print(
                f"Warning: No faces in enrollment image for {full_name}. "
                "Skipping."
            )
    ...
    if valid_embeddings == 0:
        ...
        db.commit()
        raise HTTPException(
            status_code=400, detail="No valid face embeddings found in uploaded images."
        )
