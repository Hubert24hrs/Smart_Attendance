...
async def process_frame(
    ...
    for encoding in encodings:
        ...
        if student:
            ...
            detection_count = (
                db.query(func.count(RawDetection.id))
                .filter(
            ...
            if not existing_log:
                if detection_count >= settings.REQUIRED_CONSECUTIVE_FRAMES:
                    ...
                    avg_dist = (
                        db.query(func.avg(RawDetection.distance))
                        .filter(
                            RawDetection.session_id == session_id,
                            RawDetection.student_id == student.id,
                            RawDetection.timestamp >= time_window,
                    ...
                    recognized_names.append(
                        f"⏳ {student.full_name} "
                        f"({detection_count}/"
                        f"{settings.REQUIRED_CONSECUTIVE_FRAMES} frames)"
                    )
            else:
...
def end_session(
    ...
    count = (
        db.query(AttendanceLog).filter(AttendanceLog.session_id == session_id).count()
    )
