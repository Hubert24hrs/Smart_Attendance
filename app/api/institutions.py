...
def create_institution(
    ...
    institution = Institution(
        ...
        max_students=(
            50 if data.subscription_tier == SubscriptionTier.FREE.value else 500
        ),
    )
    db.add(institution)
    db.commit()
...
def get_institution_stats(
    ...
    total_students = (
        db.query(Student).filter(Student.institution_id == institution_id).count()
    )
    total_courses = (
        db.query(Course).filter(Course.institution_id == institution_id).count()
    )
    total_teachers = (
        db.query(User)
        .filter(
            User.institution_id == institution_id, User.role == UserRole.TEACHER.value
        )
        .count()
    )
