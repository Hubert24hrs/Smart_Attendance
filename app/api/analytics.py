...
def get_analytics_overview(
    ...
    if current_user.role == UserRole.SUPER_ADMIN.value:
        sessions_query = db.query(ClassSession).filter(ClassSession.start_time >= start_date)
    else:
        sessions_query = db.query(ClassSession).join(Course).filter(
    ...
    # Attendance counts
    if current_user.role == UserRole.SUPER_ADMIN.value:
        logs_query = db.query(AttendanceLog).join(ClassSession).filter(
            ClassSession.start_time >= start_date
        )
    else:
        logs_query = db.query(AttendanceLog).join(ClassSession).join(Course).filter(
            Course.institution_id == institution_id,
            ClassSession.start_time >= start_date
        )
...
def get_daily_stats(
    ...
    for i in range(days):
        ...
        if current_user.role == UserRole.SUPER_ADMIN.value:
            ...
            day_present = db.query(AttendanceLog).join(ClassSession).filter(
                ClassSession.start_time.between(start, end),
                AttendanceLog.status == "PRESENT"
            ).count()
            day_absent = db.query(AttendanceLog).join(ClassSession).filter(
                ClassSession.start_time.between(start, end),
                AttendanceLog.status == "ABSENT"
            ).count()
        else:
            day_sessions = db.query(ClassSession).join(Course).filter(
                Course.institution_id == institution_id,
                ClassSession.start_time.between(start, end)
            ).count()
            day_present = db.query(AttendanceLog).join(ClassSession).join(Course).filter(
                Course.institution_id == institution_id,
                ClassSession.start_time.between(start, end),
                AttendanceLog.status == "PRESENT"
            ).count()
            day_absent = db.query(AttendanceLog).join(ClassSession).join(Course).filter(
                Course.institution_id == institution_id,
                ClassSession.start_time.between(start, end),
                AttendanceLog.status == "ABSENT"
            ).count()
...
def get_course_stats(
    ...
    for course in courses:
        ...
        for session in sessions:
            ...
            all_logs = db.query(AttendanceLog).filter(AttendanceLog.session_id == session.id).count()
...
def get_low_attendance_students(
    ...
    for student in students:
        total = db.query(AttendanceLog).filter(AttendanceLog.student_id == student.id).count()
        present = db.query(AttendanceLog).filter(
