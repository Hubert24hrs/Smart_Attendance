...
def view_dashboard(
    ...
    return templates.TemplateResponse(
        "dashboard.html",
        {"request": request, "user": {"username": user.username}, "sessions": sessions},
    )
...
def view_session(
    ...
    return templates.TemplateResponse(
        "report.html", {"request": request, "session": session, "logs": logs}
    )
...
def export_session_csv(
    ...
    output.seek(0)
    filename = (
        f"attendance_{session.course_name}_{session.start_time.strftime('%Y%m%d')}.csv"
    )
